...
Dataframe.place(x=0, y=0, width=400, height=400)


lblNameUserf=Label(Dataframe, text="User id: ", font=("times new roman",12, "bold"), padx=2, pady=6,bg='blue')
lblNameUserf.grid(row=0, column=0)
txtf=Entry(Dataframe, font=("arial",12, "bold"), width=12)
txtf.grid(row=0, column=1)

lblNameUserf=Label(Dataframe, text="Password: ", font=("times new roman",12, "bold"), padx=2, pady=6,bg='blue')
lblNameUserf.grid(row=1, column=0)
txtf1=Entry(Dataframe,show='*', font=("arial",12, "bold"), width=12)
txtf1.grid(row=1, column=1)


myb1=Button(Dataframe,text='Login',command=lambda:[Login1(txtf,txtf1)])
myb1.grid(row=2,column=1)


def Login1(a,b):
	if a.get()=="admin" and b.get()=="admin@123":
		messagebox.showinfo("Login","Successful")
		root.withdraw()
		Login()
	else:
		messagebox.showwarning("Login","userid/Password do not match")
		root.deiconify()
...
def Login():
	top=Toplevel()
	top.geometry("1530x720+0+0")
	top.title("Crime Scene Management")

    
	lbltitle=Label(top, bd=20, relief=RIDGE, text='Crime Scene Management', fg='black', bg="yellow", font=("times new roman",50, "bold"))
	lbltitle. pack(side=TOP, fill=X)

	#dataframe
	Dataframe=Frame(top, bd=20, relief=RIDGE,bg='yellow')
	...
	#buttons frame
	Buttonframe=Frame(top, bd=20, relief=RIDGE,bg='yellow')
	Buttonframe. place(x=0, y=530, width=1530, height=70)

	#details frame

	Detailsframe=Frame(top, bd=20, relief=RIDGE,bg='yellow')
	...
	# Display Buttons
	btnd=Button(Buttonframe,text='Display ALL',bg='green',fg='white',command=lambda: disp(top),font=("times new roman",12,"bold"),width=26,padx=1,pady=4)
	btnd.grid()
	        
	btnd1=Button(Buttonframe,text='Display ALL',bg='green',fg='white',command=lambda: disp1(top),font=("times new roman",12,"bold"),width=26,padx=1,pady=4)
	btnd1.grid(row=0,column=1)
	        
	btnd2=Button(Buttonframe,text='Display ALL',bg='green',fg='white',command=lambda: disp2(top),font=("times new roman",12,"bold"),width=26,padx=1,pady=4)
	btnd2.grid(row=0,column=2)
	        
	btnd3=Button(Buttonframe,text='Display ALL',bg='green',fg='white',command=lambda: disp3(top),font=("times new roman",12,"bold"),width=26,padx=1,pady=4)
	btnd3.grid(row=0,column=3)
	        
	btnd4=Button(Buttonframe,text='Display ALL',bg='green',fg='white',command=lambda: disp4(top),font=("times new roman",12,"bold"),width=26,padx=1,pady=4)
	btnd4.grid(row=0,column=4)

	btnd5=Button(Buttonframe,text='Display ALL',bg='green',fg='white',command=lambda: disp5(top),font=("times new roman",12,"bold"),width=26,padx=1,pady=4)
...
def update(e,a,b,d,f):
    ...
    if d.get()=="user_id":
        c1.execute("UPDATE user set user_id ="+b.get()+" WHERE user_id ="+a.get())
        c2.execute("UPDATE case2 set user_id ="+b.get()+" WHERE user_id ="+a.get())
        
    elif d.get()=="case_id":
    	c1.execute("UPDATE involves set case_id ="+b.get()+" WHERE case_id ="+a.get())
    	c2.execute("UPDATE case2 set case_id ="+b.get()+" WHERE case_id ="+a.get())

    elif d.get()=="pol_id":
    	c1.execute("UPDATE police set pol_id ="+b.get()+" WHERE pol_id ="+a.get())
    	c2.execute("UPDATE criminal set pol_id ="+b.get()+" WHERE pol_id ="+a.get())

    elif d.get()=="crm_id":
    	c1.execute("UPDATE involves set crm_id ="+b.get()+" WHERE crm_id ="+a.get())
    	c2.execute("UPDATE criminal set crm_id ="+b.get()+" WHERE crm_id ="+a.get())
...
def disp(top):
    top1=Toplevel()
    top1.geometry("1600x800+0+0")
    top1.title("USER TABLE")
    top.withdraw()
    conn=sqlite3.connect('crimeman2.db')
    c=conn.cursor()
    ...
    conn.close()
    myb1=Button(top1,text='Close window',command=lambda:[top.deiconify(),top1.destroy()])
    myb1.grid(row=i+1,column=3)
    txtent3=Entry(top1, font=("arial",12, "bold"), width=14)
    txtent3. grid(row=i+3, column=1)
    txtent3.insert(INSERT,'table name')
    txtent2=Entry(top1, font=("arial",12, "bold"), width=14)
    txtent2. grid(row=i+3, column=2)
    txtent2.insert(INSERT,'column name')
    txtent=Entry(top1, font=("arial",12, "bold"), width=14)
    txtent. grid(row=i+3, column=3)
    txtent.insert(INSERT,'Replacing id')
    txtent1=Entry(top1, font=("arial",12, "bold"), width=14)
    txtent1. grid(row=i+3, column=4)
    txtent1.insert(INSERT,'New id')
    txtent12=Entry(top1, font=("arial",12, "bold"), width=50)
    txtent12.grid(row=i+6,column=1)
    txtent12.insert(INSERT,'Query Box')
    lb=Label(top1, font=("times new roman",12, "bold"), padx=2, pady=6,bg='white',width=20)
    lb.grid(row=i+5,column=1)
    myb2=Button(top1,text='Update',command=lambda:[messagebox.showinfo("Update","Record updated"),update(txtent3,txtent,txtent1,txtent2,txtent12),top1.destroy(),disp(top)],width=20,bg='white',fg='black')
    myb2.grid(row=i+3,column=5)


def disp1(top):
    top1=Toplevel()
    top1.geometry("1600x800+0+0")
    top1.title("Case Table")
    top.withdraw()
    conn=sqlite3.connect('crimeman2.db')
    c=conn.cursor()
    ...
    lb=Label(top1, font=("times new roman",12, "bold"), padx=2, pady=6,bg='white',width=20)
    lb.grid(row=i+5,column=1)
    myb2=Button(top1,text='Update',command=lambda:[messagebox.showinfo("Update","Record updated"),update(txtent3,txtent,txtent1,txtent2,txtent12),top1.destroy(),disp1(top)],width=20,bg='white',fg='black')
    myb2.grid(row=i+3,column=5)

def disp2(top):
    top1=Toplevel()
    top1.geometry("1600x800+0+0")
    top1.title("Criminal Table")
    top.withdraw()
    conn=sqlite3.connect('crimeman2.db')
    c=conn.cursor()
    ...
    lb=Label(top1, font=("times new roman",12, "bold"), padx=2, pady=6,bg='white',width=20)
    lb.grid(row=i+5,column=1)
    myb2=Button(top1,text='Update',command=lambda:[messagebox.showinfo("Update","Record updated"),update(txtent3,txtent,txtent1,txtent2,txtent12),top1.destroy(),disp2(top)],width=20,bg='white',fg='black')
    myb2.grid(row=i+3,column=5)

def disp3(top):
    top1=Toplevel()
    top1.geometry("1600x800+0+0")
    top1.title("Crime Scene Table")
    top.withdraw()
    conn=sqlite3.connect('crimeman2.db')
    c=conn.cursor()
    ...
    lb=Label(top1, font=("times new roman",12, "bold"), padx=2, pady=6,bg='white',width=20)
    lb.grid(row=i+5,column=1)
    myb2=Button(top1,text='Update',command=lambda:[messagebox.showinfo("Update","Record updated"),update(txtent3,txtent,txtent1,txtent2,txtent12),top1.destroy(),disp3(top)],width=20,bg='white',fg='black')
    myb2.grid(row=i+3,column=5)
...
def disp4(top):
    ...
    lb=Label(top1, font=("times new roman",12, "bold"), padx=2, pady=6,bg='white',width=20)
    lb.grid(row=i+5,column=1)
    myb2=Button(top1,text='Update',command=lambda:[messagebox.showinfo("Update","Record updated"),update(txtent3,txtent,txtent1,txtent2,txtent12),top1.destroy(),disp4(top)],width=20,bg='white',fg='black')
    myb2.grid(row=i+3,column=5)
...
def disp5(top):
    ...
    lb=Label(top1, font=("times new roman",12, "bold"), padx=2, pady=6,bg='white',width=20)
    lb.grid(row=i+5,column=1)
    myb2=Button(top1,text='Update',command=lambda:[messagebox.showinfo("Update","Record updated"),update(txtent3,txtent,txtent1,txtent2,txtent12),top1.destroy(),disp5(top)],width=20,bg='white',fg='black')
    myb2.grid(row=i+3,column=5)
